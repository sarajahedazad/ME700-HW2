...
class ShapeFunctions:
    ...
    def plot_element_interpolation( self ):
        ...
        element_interpolated = self.calc_element_interpolation( 0 )
        p0 = element_interpolated[0]
        p1 = element_interpolated[1]
        ax.plot([p0[0], p1[0]], [p0[1], p1[1]], [p0[2], p1[2]], 'r', label='Shape')
        for i, _ in enumerate( element_interpolated[ : -2 ] ) :
            p0 = element_interpolated[ i + 1 ]
            p1 = element_interpolated[ i + 2 ]
            ax.plot([p0[0], p1[0]], [p0[1], p1[1]], [p0[2], p1[2]], 'r')
        for j in range( 1, n_connectivities):
            element_interpolated = self.calc_element_interpolation( j )
            for i, _ in enumerate( element_interpolated[ : - 1] ) :
